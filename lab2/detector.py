...
PORT_THRESHOLD = int(os.getenv("NETWORK_SCAN_PORT_THRESHOLD", 4))
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if "network scan detected" in normalized:
            ...
            if "ports:" in normalized:
                ...
                ports_list = [p.strip() for p in line.split("ports")[-1].split(",") if p.strip()]
                num_ports = len(ports_list)
                ...
                findings.append(
                    {
                        ...
                        "detection_rule": (
                            f"The log contains 'network scan detected' with {num_ports} ports (Threshold: {PORT_THRESHOLD})."
                        ),
